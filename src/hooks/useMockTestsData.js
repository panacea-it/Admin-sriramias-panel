...
export function useMockTestsData({ enabled = true, search = '', page = 1, limit = 10 } = {}) {
  ...
  const loadMockTests = useCallback(
    async ({ searchQuery, pageNo = page, pageLimit = limit } = {}) => {
      ...
      try {
        ...
        const normalized = normalizeMockTestsListResponse(data, {
          page: pageNo,
          limit: pageLimit,
        })
        ...
        const message = getMockTestApiErrorMessage(err, 'Failed to load mock tests.')
        setError(message)
        setItems([])
      } finally {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      loadMockTests({ searchQuery: search })
    }, 300)
  ...
  }, [enabled, search, loadMockTests])

  const refresh = useCallback(() => {
    loadMockTests({ searchQuery: search, pageNo: page, pageLimit: limit })
  }, [loadMockTests, search, page, limit])
