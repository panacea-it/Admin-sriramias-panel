...
export default function ConfirmTopicStatusModal({
  ...
  const title = enabling ? 'Enable topic?' : 'Disable topic?'
  const description = enabling
    ? `"${topicName}" will be marked active and available in dependent dropdowns.`
    : `"${topicName}" will be marked inactive and hidden from operational dropdowns until re-enabled.`
    ...
    <Modal open={open} onClose={onCancel} title={title} size="md">
      <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl">
        ...
        <div className="flex flex-col-reverse gap-3 px-6 py-5 sm:flex-row sm:justify-end">
          ...
          >
            {loading ? 'Updating…' : enabling ? 'Enable topic' : 'Disable topic'}
          </button>
        </div>
      </div>
    </Modal>
