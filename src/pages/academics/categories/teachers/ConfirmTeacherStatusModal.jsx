...
export default function ConfirmTeacherStatusModal({
  ...
  const title = enabling ? 'Enable teacher?' : 'Disable teacher?'
  const description = enabling
    ? `"${teacherName}" will be marked active and available in dependent dropdowns.`
    : `"${teacherName}" will be marked inactive and hidden from operational dropdowns until re-enabled.`
    ...
    <Modal open={open} onClose={onCancel} title={title} size="md">
      <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl">
        ...
        <div className="flex flex-col-reverse gap-3 px-6 py-5 sm:flex-row sm:justify-end">
          ...
          >
            {loading ? 'Updating…' : enabling ? 'Enable teacher' : 'Disable teacher'}
          </button>
        </div>
      </div>
    </Modal>
