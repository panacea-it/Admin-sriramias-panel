...
function buildCounselorsByCenter() {
  ...
  return Object.fromEntries(LEAD_CENTERS.map((center) => [center, options]))
}
...
export default function LeadsPage() {
  ...
  const enrichLead = useCallback(
    (row) => ({
      ...
      assignedCounselorName: counselorById[row.id] || '',
      status: statusById[row.id] || '',
      courseVisited:
        row.courseVisited ||
        [row.course, row.courseSub].filter(Boolean).join(' ').trim() ||
        '—',
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter((row) => {
      ...
      const matchDate = leadMatchesSelectedDate(row, dateFilter)
      return matchSearch && matchCenter && matchStatus && matchDate
    })
  }, [leads, search, centerFilter, statusFilter, statusById, dateFilter])
  ...
  const handleView = useCallback(
    (row) => setViewLead(enrichLead(row)),
    [enrichLead],
  )

  const handleEdit = useCallback(
    (row) => setEditLead(enrichLead(row)),
    [enrichLead],
  )
    ...
    <div className="figma-admin-section min-h-screen bg-[#f7f7f7] px-4 pb-8 pt-6 sm:px-5 lg:px-6">
      ...
      {editLead && (
        <LeadEditModal
          ...
          lead={editLead}
          counselorOptions={
            COUNSELORS_BY_CENTER[editLead.center] || [
