...
function DateCell({ time, date }) {
  return (
    <div className="leading-snug">
      ...
      <span className="mt-0.5 block whitespace-nowrap text-xs text-[#686868]">
        {date}
      </span>
    </div>
  );
...
export default function LeadsTable({
  ...
  counselorById,
  statusById,
  counselorsByCenter, // NEW DICTIONARY PROP
  statusOptions,
  onCounselorChange,
  onStatusChange,
  onView,
    ...
    () => [
      {
        ...
        cellClassName: CELL,
        render: (row) => (
          <TextCell value={row.userName} className="font-semibold" />
        ),
      },
      {
        key: "email",
      ...
      },
      {
        ...
        render: (row) => (
          <CourseCell
            course={row.courseVisited || row.course}
