...
const cards = [
  {
    label: "Total Leads",
    key: "totalLeads", // Changed from 'total' to match API
...
export default function LeadStatCards({ stats }) {
  return (
    // Updated grid classes to stretch 4 cards across the screen perfectly
      ...
      {cards.map(({ label, key, valueColor, accent, bg }) => (
        ...
        >
          ...
          <div className="flex items-center justify-between gap-3 pl-2">
            <span className="text-sm font-semibold text-[#333] sm:text-base">
              {label}
            </span>
            <span
              className={cn(
            ...
            >
              {/* Safely fallback to '0' if the API hasn't loaded yet */}
              {stats?.[key] !== undefined
