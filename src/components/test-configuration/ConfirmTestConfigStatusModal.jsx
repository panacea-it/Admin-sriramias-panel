...
export default function ConfirmTestConfigStatusModal({
  open,
  entityLabel,
  enabling,
  loading,
  ...
  const title = enabling ? `Enable ${entityLabel}?` : `Disable ${entityLabel}?`
  const message = enabling
    ? `Are you sure you want to enable this ${entityLabel.toLowerCase()}?`
    : `Are you sure you want to disable this ${entityLabel.toLowerCase()}?`
  const confirmLabel = enabling ? 'Enable' : 'Disable'
