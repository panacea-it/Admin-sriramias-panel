...
export default function PrelimsLanguagePdfUploadCard({
  language,
  fileName = '',
  fileSize = null,
  pdfUrl = '',
  ...
  const hasFile = Boolean((pdfUrl || fileName) && fileName)
  ...
  const applyFile = async (file) => {
    ...
    try {
      ...
      setValidationError('Failed to process PDF. Please try again.')
      setUploadStatus('')
    } finally {
      setUploading(false)
    }
  ...
  return (
    <div
      className={cn(
        'rounded-2xl border border-[#e5eaf2] bg-white p-4 shadow-sm sm:p-5',
        error && 'border-red-300',
      )}
    >
      <div className="mb-4">
        <h5 className="text-sm font-bold text-[#1a3a5c] sm:text-base">
          {language} Question Paper
        </h5>
        <p className="mt-0.5 text-xs text-[#686868]">
          Upload PDF for {language} Language
        </p>
      </div>
      ...
      {hasFile ? (
        <div
          className={cn(
            'rounded-2xl border border-[#55ace7]/20 bg-[#f8fbff] p-4',
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <ActionButton
                icon={Eye}
                label="View PDF"
                onClick={() => viewBrochurePdf(pdfUrl)}
                disabled={disabled || uploading || !pdfUrl}
              />
              <ActionButton
                icon={Download}
                label="Download"
                onClick={() => downloadBrochurePdf(pdfUrl, fileName)}
                disabled={disabled || uploading || !pdfUrl}
              />
              <ActionButton
                icon={RefreshCw}
          ...
          <div className="flex min-h-[160px] flex-col items-center justify-center gap-3 px-6 py-8 text-center">
            {uploading ? (
              ...
              <>
                ...
                <span className="text-base font-semibold text-[#246392]">Drag & Drop PDF</span>
                <span className="max-w-sm text-sm text-gray-500">or browse to select a file</span>
                <button
                  type="button"
                  onClick={() => inputRef.current?.click()}
                  disabled={disabled}
