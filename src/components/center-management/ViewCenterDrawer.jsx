...
export default function ViewCenterDrawer({ open, centerId, onClose }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const apiResponse = await getCenterById(centerId);
        const mapped = normalizeCenterDetailsResponse(apiResponse);
        if (!cancelled) {
          setCenter(mapped);
        }
      } catch (error) {
  ...
  const admins =
    center?.assignedAdminsDisplay?.trim() ||
    (center?.assignedAdmins?.length > 0
      ? center.assignedAdmins.join(", ")
      : "None assigned");
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="sm:col-span-2">
                <Row
                  icon={MapPin}
                  label="Address"
                  value={[center.address, center.city, center.state]
                    .filter(Boolean)
                    .join(", ")}
                />
              </div>
              ...
              {center.createdByLabel ? (
                <Row
                  icon={Building2}
                  label="Created by"
                  value={center.createdByLabel}
                />
              ) : null}
