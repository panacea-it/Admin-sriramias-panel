...
import { createPdfAsset } from '../../../utils/bookstoreProductForm'
...
const PDF_PROFILE = UPLOAD_PROFILES.BOOKSTORE_PREVIEW_PDF

export default function SamplePdfUpload({ value, onChange, onUploadStart, error }) {
  ...
  const applyFile = async (file) => {
    if (!file) return
    const result = await validateUploadFile(file, 'BOOKSTORE_PREVIEW_PDF')
    if (!result.valid) {
      setValidationError(result.message)
      return
    }
    setValidationError(null)
    const asset = createPdfAsset(file)
    onChange(asset)
  ...
  return (
    <div className="w-full">
        ...
        {value?.previewUrl ? (
          <div className="flex min-h-[220px] flex-col items-center justify-center gap-4 p-6">
            <span className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#ede8f8]">
              <FileText className="h-8 w-8 text-[#7c5cbf]" strokeWidth={1.8} />
            </span>
            <p className="max-w-md text-center text-sm font-semibold text-[#3d2d6b]">
              {value.fileName || 'Sample PDF'}
            </p>
            ...
            {!value.uploading && value.previewUrl && !value.previewUrl.startsWith('blob:') ? (
              <iframe
                title={value.fileName || 'Sample PDF preview'}
                src={value.previewUrl}
                className="h-48 w-full max-w-lg rounded-xl border border-[#e8ecf2] bg-white"
              />
            ) : null}
          ...
          >
            ...
            <span className="text-base font-semibold text-[#3d2d6b]">Drop sample PDF here</span>
            <span className="max-w-md text-sm text-[#686868]">or click to browse</span>
            <span className="text-xs font-medium text-[#9ca0a8]">Single PDF only</span>
          </button>
        )}
      ...
      <UploadFieldHint profile={PDF_PROFILE} className={BOOKSTORE_HELPER_CLASS} />
      <p className={BOOKSTORE_HELPER_CLASS}>
        Optional sample or preview PDF for students to read before purchase.
      </p>
