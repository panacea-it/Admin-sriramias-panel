...
export default function ConfirmProgramStatusModal({
  open,
  programName,
  enabling,
  loading,
  ...
  const title = enabling ? 'Enable program?' : 'Disable program?'
  const description = enabling
    ? `This will mark "${programName}" as active.`
    : `This will mark "${programName}" as inactive.`
    ...
    <Modal open={open} onClose={onCancel} title={title} size="md">
      <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xl">
        ...
        <div className="flex flex-col-reverse gap-3 px-6 py-5 sm:flex-row sm:justify-end">
          ...
          >
            {loading ? 'Updating…' : enabling ? 'Enable program' : 'Disable program'}
          </button>
        </div>
      </div>
    </Modal>
