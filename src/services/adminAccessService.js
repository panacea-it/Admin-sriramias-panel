...
export function mapApiAdminToRow(data) {
  ...
  return {
    ...
    roleId,
    roleTitle:
      String(role?.roleTitle || role?.label || data.roleTitle || "").trim() ||
      "—",
    centerId,
    centerName:
...
export function buildAdminAccessPayload(
  form,
  { isEdit = false, includePassword = true } = {},
) {
  const payload = {
...
export function mapAdminUserToForm(data, defaults = {}) {
  ...
  return {
    ...
    roleId: String(
      role?._id || role?.id || user.roleId || defaults.roleId || "",
    ),
    centerId: String(
      center?._id || center?.id || user.centerId || defaults.centerId || "",
    ),
    password: "",
    confirmPassword: "",
    ...
    loginAlert:
      user.loginAlertEnabled !== undefined
        ? Boolean(user.loginAlertEnabled)
        : true,
  };
}
...
export const updateAdminUser = async (adminAccessId, payload) => {
  try {
    const response = await api.put(
      `/api/admin/admin-access/${adminAccessId}`,
      payload,
    );
    return response.data;
  } catch (error) {
...
export const updateAdminStatus = async (adminAccessId, status) => {
  try {
    const response = await api.patch(
      `/api/admin/admin-access/${adminAccessId}/status`,
      {
