...
function normalizeRole(raw, item = {}) {
  ...
  const codeValue = String(
    item?.roleType || item?.userType || item?.roleKey || rawValue || "",
  )
    .trim()
    .toUpperCase();

  if (labelValue.includes("student") || codeValue.includes("STUDENT"))
    return "student";
  if (
    labelValue.includes("teacher") ||
    labelValue.includes("faculty") ||
    codeValue === "TEA"
  )
    return "faculty";
  if (labelValue.includes("mentor") && labelValue.includes("admin"))
    return "mentor_admin";
  if (labelValue.includes("counselor") || labelValue.includes("parent"))
    return "counselor";
  if (labelValue.includes("employee") || codeValue.includes("EMPLOYEE"))
    return "employee";
  if (labelValue.includes("support") || labelValue.includes("staff"))
    return "support_staff";
  if (labelValue.includes("admin") || codeValue.includes("ADMIN"))
    return "admin";
...
function isStudentRecord(item) {
  const roleValue = String(
    item?.roleKey || item?.roleType || item?.userType || item?.role || "",
  )
    .trim()
    .toLowerCase();

  const recordType = String(item?.recordType || item?.userType || "")
    .trim()
    .toUpperCase();
...
function mapApiUserToRow(item) {
  ...
  const studentDetails =
    item.studentDetails && typeof item.studentDetails === "object"
      ? item.studentDetails
      : {};
  ...
  return {
    ...
    roleLabel:
      String(item.role || item.roleTitle || item.userType || "").trim() ||
      "User",
    userId: isStudent
      ? studentId || String(item.userId || "").trim() || ""
      : "",
    assignedCenter:
      String(
        item.centerName || item.center || item.assignedCenter || "-",
      ).trim() || "-",
    ...
    parentName:
      String(studentDetails.parentName || item.parentName || "").trim() || null,
    parentPhone:
      String(
...
export function normalizeUnifiedUsersResponse(
  data,
  { page = 1, limit = 10 } = {},
) {
  const payload =
...
function mapStatusFilterToApi(statusFilter) {
  const normalized = String(statusFilter || "")
    .trim()
    .toUpperCase();
  if (
    normalized === "ACTIVE" ||
