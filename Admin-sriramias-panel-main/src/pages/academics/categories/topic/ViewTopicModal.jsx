...
export default function ViewTopicModal({ open, onClose, item, loading = false }) {
  if (!open) return null

  if (loading) {
    return (
      <Modal open={open} onClose={onClose} size="md" title="View topic">
      ...
      <div className="overflow-hidden rounded-2xl bg-white shadow-[0_24px_60px_rgba(15,23,42,0.22)]">
        <header className="flex items-start justify-between gap-3 bg-gradient-to-r from-[#55ace7] via-[#5a7ba8] to-[#1a3a5c] px-5 py-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0 text-white">
              ...
              <p className="text-sm text-white/85">{item.displayId || item.topicId || item.id}</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-4 p-5 sm:p-6">
          ...
          <dl className="grid gap-4 sm:grid-cols-2">
            <DetailItem label="ID">{item.displayId || item.topicId || item.id}</DetailItem>
            <DetailItem label="Topic">{item.name}</DetailItem>
